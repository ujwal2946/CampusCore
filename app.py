...
def init_db():
    ...
    if c.fetchone()[0] == 0:
        ...
        c.execute('INSERT INTO users (username, password, role) VALUES (?, ?, ?)', 
                 ('UKVM', '501', 'admin'))
        # Create attendance staff user
        c.execute('INSERT INTO users (username, password, role) VALUES (?, ?, ?)',
                 ('track', '2946', 'attendance'))
        conn.commit()
    else:
        # Check if attendance staff user exists, if not create it
        c.execute('SELECT COUNT(*) FROM users WHERE username = ?', ('track',))
        if c.fetchone()[0] == 0:
            c.execute('INSERT INTO users (username, password, role) VALUES (?, ?, ?)',
                     ('track', '2946', 'attendance'))
            conn.commit()
